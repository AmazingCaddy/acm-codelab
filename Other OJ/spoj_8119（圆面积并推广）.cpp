/*
	author: AmazingCaddy
	time: 2011/3/5  20:58
	题意：给出N个不同的圆，求出被覆盖恰好K次的面积，并顺序输出
	至少覆盖一次的，至少覆盖两次的，一直到至少覆盖n次的。。。然后做差
*/
#include <cstdio>
#include <complex>
#include <cstdlib>
...
void areaUnion()
{
	...
	for(i = 0; i < n; i++)
	{
		...
		for(j = 0; j < k; j++)
		{
			...
			area[cnt] += cir[i].r * cir[i].r *( a - sin(a) );
			area[cnt] += E[j].p.x * E[j+1].p.y - E[j].p.y * E[j+1].p.x;
		}
	}
	for( i = 0; i < maxn; i++ )
		area[ i ] /= 2.0;
